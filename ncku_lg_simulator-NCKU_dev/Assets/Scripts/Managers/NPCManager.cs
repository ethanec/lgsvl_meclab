...
public class NPCManager : MonoBehaviour
{
    ...
    private void SetNPCOnMap()
    {
        for (int i = 0; i < CurrentPooledNPCs.Count; i++)
        {
            ...
            // Judge a car or a motor and geting spawn lane
            MapLane lane;
            if (CurrentPooledNPCs[i].name.Contains("Motor"))
            {
                lane = mapManager.GetMotorLane(RandomGenerator.Next(mapManager.motorTrafficLanes.Count));
            }
            else
            {
                lane = mapManager.GetLane(RandomGenerator.Next(mapManager.trafficLanes.Count));
            }
            ...
            /* Not suitable for NCKU simulator
