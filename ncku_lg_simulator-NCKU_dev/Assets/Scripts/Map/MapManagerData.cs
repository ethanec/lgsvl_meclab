...
namespace Simulator.Map
{
    public class MapManagerData
    {
        ...
        public List<MapLane> GetTrafficLanes()
        {
            ...
            var CarTrafficLanes = new List<MapLane>();
            foreach (var lane in trafficLanes)
            {
                if(!lane.motorOnly)
                {
                    CarTrafficLanes.Add(lane);
                    CarTrafficLanes[CarTrafficLanes.Count - 1].isTrafficLane = true;
                }
            }
            trafficLanes = CarTrafficLanes;
        ...
        public List<MapLane> GetMotorTrafficLanes()
        {   
            var trafficLanesHolder = MapHolder.trafficLanesHolder;
            var trafficLanes = new List<MapLane>(trafficLanesHolder.GetComponentsInChildren<MapLane>());
            var MotorTrafficLanes = new List<MapLane>();
            foreach (var lane in trafficLanes)
            {
                if(!lane.vehicleOnly) 
                {
                    MotorTrafficLanes.Add(lane);
                    MotorTrafficLanes[MotorTrafficLanes.Count - 1].isTrafficLane = true;
                }
            }
            return MotorTrafficLanes;
        }
            ...
            foreach (var lane in lanes) // set connected lanes and spawnable
            {
                ...
                foreach (var altLane in lanes)
                {
                    ...
                    if ((lastPt - altFirstPt).magnitude < ConnectionProximity)
                    {
                        ...
                        if (!altLane.motorOnly) lane.nextConnectedVehicleLanes.Add(altLane);
                        if (!altLane.vehicleOnly) lane.nextConnectedMotorLanes.Add(altLane); 
