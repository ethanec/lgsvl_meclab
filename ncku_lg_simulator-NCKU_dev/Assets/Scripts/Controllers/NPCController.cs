...
public class NPCController : MonoBehaviour
{
    ...
    private RaycastHit overtakeLeftDectionHitInfo = new RaycastHit();
    private RaycastHit overtakeRightDectionHitInfo = new RaycastHit();
    ...
    // targeting
    private Transform overtakeRightDection;
    private Transform overtakeLeftDection;
        ...
        switch (Control)
        {
            case ControlType.Automatic:
                if (isLaneDataSet)
                {
                    ...
                    if (gameObject.name.Contains("Motor")) CheckLaneChange();
                }
                break;
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        //For NCKU Motor Overtake Scenario 
        if (gameObject.name.Contains("Motor") &  other.gameObject.layer == 0)
        {
            for (int i = 0; i < 2; i++)
            {
    ...
    private void SetNeededComponents()
    {
        ...
        //Set wheel radius for motor
        if (gameObject.name.Contains("Motor")){
            wheelColliderRR.radius = 0.3f;
            wheelColliderRL.radius = 0.3f;
            wheelColliderFL.radius = 0.3f;
            wheelColliderFR.radius = 0.3f;
        }
        ...
        // add transform for detect obstacle
        if (gameObject.name.Contains("Motor"))
        {
            go = new GameObject("OvertakeRightDection");
            go.transform.position = new Vector3(Bounds.center.x, Bounds.min.y + 0.5f, Bounds.center.z);
            go.transform.SetParent(transform, true);
            overtakeRightDection = go.transform;
            go = new GameObject("OvertakeLeftDection");
            go.transform.position = new Vector3(Bounds.center.x , Bounds.min.y + 0.5f, Bounds.center.z);
            go.transform.SetParent(transform, true);
            overtakeLeftDection = go.transform;
        }
        normalSpeed = RandomGenerator.NextFloat(laneSpeedLimit - 3 + aggression, laneSpeedLimit + 1 + aggression);
    }
    ...
    private void GetNextLane()
    {
        ...
        if (gameObject.name.Contains("Motor"))
        {
            // For more realistic and chaotic motor behaviors
            MapLane LeftLane = currentMapLane.leftLaneForward; 
            MapLane RightLane = currentMapLane.rightLaneForward;
            MapLane tempMapLaneLeft = currentMapLane; 
            MapLane tempMapLaneRight = currentMapLane;
            float rand; // A random number making motor behaviors more chaotic
            int i; // Counter of MapLane's nodes           
            if (currentMapLane?.nextConnectedMotorLanes.Count >= 1) // choose next path and set waypoints
            {
                currentMapLane = currentMapLane.nextConnectedMotorLanes[RandomGenerator.Next(currentMapLane.nextConnectedMotorLanes.Count)];
                // If LeftLane exists, we compute and get a new MapLane.                       
                if (LeftLane != null && !isFrontLeftDetect)
                {                   
                    for (i = 0; i < tempMapLaneLeft.mapLocalPositions.Count; i++)
                    {
                        rand = Random.Range(0.2f , 0.8f);
                        tempMapLaneLeft.mapWorldPositions[i] = new Vector3((float)(tempMapLaneLeft.mapWorldPositions[i].x*rand + LeftLane.mapWorldPositions[i].x*(1.0-rand)), (float)(tempMapLaneLeft.mapWorldPositions[i].y), (float)(tempMapLaneLeft.mapWorldPositions[i].z*rand + LeftLane.mapWorldPositions[i].z*(1.0-rand)));
                        // Get nodes of tempMapLaneLeft.                                        
                    }
                    if (currentMapLane.transform.parent.gameObject.name.Contains("TrafficLanes"))
                        // When current MapLane belongs to intersection, quit this function. Or wrong behaviors will happen.                   
                        currentMapLane = tempMapLaneLeft;                    
                }
                // Initially, we detect left, and then right.
                else if (RightLane != null && !isFrontRightDetect)
                {                 
                    for (i = 0; i < tempMapLaneRight.mapLocalPositions.Count; i++)
                    {                        
                        rand = Random.Range(0.2f , 0.8f);
                        tempMapLaneRight.mapWorldPositions[i] = new Vector3((float)(tempMapLaneRight.mapWorldPositions[i].x*rand + RightLane.mapWorldPositions[i].x*(1.0-rand)), (float)(tempMapLaneRight.mapWorldPositions[i].y), (float)(tempMapLaneRight.mapWorldPositions[i].z*rand + RightLane.mapWorldPositions[i].z*(1.0-rand)));
                                             
                    }
                    if (currentMapLane.transform.parent.gameObject.name.Contains("TrafficLanes"))                    
                        currentMapLane = tempMapLaneRight;                     
                }             
        ...
        {
            if (currentMapLane?.nextConnectedVehicleLanes.Count >= 1) // choose next path and set waypoints
            {
                currentMapLane = currentMapLane.nextConnectedVehicleLanes[RandomGenerator.Next(currentMapLane.nextConnectedVehicleLanes.Count)];
                laneSpeedLimit = currentMapLane.speedLimit;
                aggressionAdjustRate = laneSpeedLimit / 11.176f; // 11.176 m/s corresponds to 25 mph
                normalSpeed = RandomGenerator.NextFloat(laneSpeedLimit - 3 + aggression, laneSpeedLimit + 1 + aggression);
                SetLaneData(currentMapLane.mapWorldPositions);
    ...
    {
        //Check obstacles ahead and decide to overtake
        if (isLaneChanging) return;
        if (frontClosestHitInfo.collider && frontClosestHitInfo.distance < 20f)
        {
            if(Random.Range(1.0f, 10.0f) > 7.0f)
            {
                isLaneChanging = true;
                MotorLaneChange();
            }
    ...
    private void MotorLaneChange()
    {
        bool FindTheLane = false;
        MapLane LeftLane = currentMapLane.leftLaneForward;
        MapLane RightLane = currentMapLane.rightLaneForward;     
        Vector3 LeftPos = new Vector3(overtakeLeftDection.position.x , overtakeLeftDection.position.y , overtakeLeftDection.position.z);
        Vector3 RightPos = new Vector3(overtakeRightDection.position.x , overtakeRightDection.position.y , overtakeRightDection.position.z);
        float distance;
        int count = 2;  //count為 往外偵測幾個車道
        while (!FindTheLane && count > 0)
        {
            // check left lane first
            if (LeftLane != null && !isFrontLeftDetect)
            {
                // check obstscle 
                distance = (currentMapLane.transform.position - currentMapLane.leftLaneForward.transform.position).magnitude;
                overtakeLeftDection.localPosition = new Vector3(overtakeLeftDection.localPosition.x - distance , overtakeLeftDection.localPosition.y , overtakeLeftDection.localPosition.z);
                CollisionCheck();
                if (overtakeLeftDectionHitInfo.distance == 0)
                {
                    currentMapLane = LeftLane;
                    laneSpeedLimit = LeftLane.speedLimit;
                    aggressionAdjustRate = laneSpeedLimit / 11.176f; // 11.176 m/s corresponds to 25 mph
                    SetChangeLaneData(currentMapLane.mapWorldPositions);
                    Coroutines[(int)CoroutineID.DelayOffTurnSignals] = FixedUpdateManager.StartCoroutine(DelayOffTurnSignals());
                    FindTheLane = true;
                ...
                {
                    LeftLane = LeftLane.leftLaneForward;
                }
            }
            // check right lane
            if (RightLane != null && FindTheLane == false && !isFrontRightDetect)
            {
                distance = (currentMapLane.transform.position - currentMapLane.rightLaneForward.transform.position).magnitude;
                overtakeRightDection.localPosition = new Vector3(overtakeRightDection.localPosition.x + distance , overtakeRightDection.localPosition.y, overtakeRightDection.localPosition.z);
                CollisionCheck();
                if (overtakeRightDectionHitInfo.distance == 0)
                {
                    currentMapLane = RightLane;                    
                    laneSpeedLimit = RightLane.speedLimit;
                    aggressionAdjustRate = laneSpeedLimit / 11.176f; // 11.176 m/s corresponds to 25 mph
                    SetChangeLaneData(currentMapLane.mapWorldPositions);
                    Coroutines[(int)CoroutineID.DelayOffTurnSignals] = FixedUpdateManager.StartCoroutine(DelayOffTurnSignals());
                    FindTheLane = true;
                ...
                {
                    RightLane = RightLane.rightLaneForward;
                } 
            }
            count--;
        }
        overtakeLeftDection.position = LeftPos;
        overtakeRightDection.position = RightPos;
    }
    ...
    private IEnumerator DelayOffTurnSignals()
    {
        ...
        isLeftTurn = isRightTurn = isLaneChanging = false;
        SetNPCTurnSignal();
    }
    ...
    {
        if (gameObject.name.Contains("Motor")) return;
    ...
    private void CollisionCheck()
    {
        ...
        if (!Physics.Raycast(transform.position + Vector3.up, Vector3.down, out groundCheckInfo, 5f, groundHitBitmask))
            Despawn();
        // use for motor switching lane 
        if (gameObject.name.Contains("Motor"))
        {
            if  ( overtakeLeftDection == null || overtakeRightDection == null) return;
            overtakeLeftDectionHitInfo = new RaycastHit();
            overtakeRightDectionHitInfo = new RaycastHit();
            Physics.Raycast(overtakeLeftDection.position, overtakeLeftDection.forward, out overtakeLeftDectionHitInfo, frontRaycastDistance, carCheckBlockBitmask);
            Physics.Raycast(overtakeRightDection.position, overtakeRightDection.forward, out overtakeRightDectionHitInfo, frontRaycastDistance, carCheckBlockBitmask);
        }
    ...
    public void SetFollowClosestLane(float maxSpeed, bool isLaneChange)
    {
        ...
        if (gameObject.name.Contains("Motor")) lane = SimulatorManager.Instance.MapManager.GetClosestLane(position, "motor");
        InitLaneData(lane);
