...
{
    [SensorType("GPS NCKU Device", new[] { typeof(GpsNCKUData) })]
    public class GpsNCKUSensor : SensorBase
    {
        [SensorParameter]
        [Range(1f, 100f)]
        ...
        {
            Bridge = bridge;
            Writer = Bridge.AddWriter<GpsNCKUData>(Topic);
        }
        ...
        void FixedUpdate()
        {
            ...
            var data = new GpsNCKUData()
            {
				...
				ENU_x = Vehicle.transform.position.x,
				ENU_y = Vehicle.transform.position.z,
				ENU_z = Vehicle.transform.position.y
            };
